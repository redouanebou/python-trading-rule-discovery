...
FEATURES_FILE_PATH = "features_GBP.csv"
...
def validate_rule_performance(indices, open_prices, high_prices, low_prices, atr_values, trade_type):
    ...
    with an ATR-based SL and TP to see what the real outcome was.
    """
...
if __name__ == "__main__":
    print("🚀 Starting Phase 2 (HONEST): Rule Discovery for GBPUSD...")
    ...
    print(f"\n🔥 Starting {ITERATIONS} brute-force iterations...")
    found_buy_rules = []
    found_sell_rules = []
    class_names = {0: 'No_Trade', 1: 'Buy', -1: 'Sell'}

    for i in tqdm(range(ITERATIONS), desc="Discovering HONEST Rules"):
    ...
    for i, rule in enumerate(found_buy_rules):
        rule['rule_id'] = f"HONEST_GBPUSD_BUY_{i+1:03d}"
    for i, rule in enumerate(found_sell_rules):
        rule['rule_id'] = f"HONEST_GBPUSD_SELL_{i+1:03d}"
